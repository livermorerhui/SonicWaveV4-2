...
- 步骤：
  ...
  4. 下载完成后，将音频写入本地文件目录，并加入本地音乐列表。
  5. 播放该音乐，随后关闭网络，尝试再次播放。
- 预期结果：
  - 下载过程中如网络错误，APP 给出明确的错误提示，不影响已有本地音乐播放。
  - 下载成功后，在线音乐出现在本地音乐列表中，可正常播放。
  - 关闭网络后，已下载的音乐仍然可以播放，证明缓存生效。
...
### 用例 5-3：后台控制消息生效（强制切换运行模式）

- 步骤：
  1. 启动 APP，并保证其与后端控制通道（例如 WebSocket）成功建立连接。
  2. 将 APP 切换到离线模式或特定测试模式。
  3. 在管理端 Web 中，针对当前设备下发一条控制指令，例如“强制退出离线模式”或“切换回在线模式”。
  4. 观察 APP 的界面和状态变化。
- 预期结果：
  - 控制通道连接正常时，APP 能够在合理时间内（例如数秒内）接收到该指令。
  - APP 正确解析指令内容，并根据策略执行相应操作（例如自动切回在线模式、关闭某些功能）。
  - 对应的状态变化有明显的界面反馈或日志记录，便于后续排查。
  - 若控制通道断开或指令解析失败，APP 至少记录错误日志，不出现崩溃。
